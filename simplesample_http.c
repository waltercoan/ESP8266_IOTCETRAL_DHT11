...
DECLARE_MODEL(ContosoSensor,
WITH_DATA(ascii_char_ptr, DeviceId),
WITH_DATA(float, temp),
WITH_DATA(float, umid)
);
...
void simplesample_http_run(int temperature, int humidity)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                myWeather = CREATE_MODEL_INSTANCE(WeatherStation, ContosoSensor);
                if (myWeather == NULL)
                {
                ...
                else
                {
                    ...
                    {
                        myWeather->DeviceId = "50a96c0f-c282-4474-aa73-1bb4e8946a8b";
                        myWeather->temp = temperature;
                        myWeather->umid = humidity;
                        {
                            unsigned char* destination;
                            size_t destinationSize;
                            if (SERIALIZE(&destination, &destinationSize, myWeather->DeviceId, myWeather->temp, myWeather->umid) != CODEFIRST_OK)
                            {
...
void sample_run(int temperature, int humidity)
{
    simplesample_http_run(temperature, humidity);
}
